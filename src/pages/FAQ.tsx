...
const faqs = [
  {
    ...
    items: [
      { q: 'Is my recording uploaded to your servers?', a: 'No. ScreenCraft is built on a 100% client-side architecture. Your recording data never leaves your device. The MediaRecorder API captures and encodes video entirely in your browser memory. We have no servers that receive video data — it\'s architecturally impossible for us to see your recordings.' },
      ...
      { q: 'Is ScreenCraft safe to use for sensitive content?', a: 'Yes. Because recordings never leave your device, ScreenCraft is safe for recording sensitive content like financial dashboards, medical information, confidential business data, or any other material you would not want transmitted to third parties. The architecture enforces privacy — not just policy.' },
    ],
  },
  {
    category: 'Recording Features',
    items: [
      { q: 'How does browser-based recording work?', a: 'Modern browsers (Chrome, Edge, Firefox) expose native APIs — getDisplayMedia for screen capture, getUserMedia for webcam and microphone, and MediaRecorder for encoding. ScreenCraft uses these standard browser APIs to give you a full recording experience without any software installation. Everything happens in JavaScript running in your browser tab.' },
      ...
      { q: 'Can I record my webcam and screen at the same time?', a: 'Yes. Choose "Screen + Webcam" or "Screen + Webcam + Audio" mode. ScreenCraft captures both your screen and your webcam stream simultaneously. The webcam feed is included as a video track in the recording.' },
      { q: 'Is there a time limit on recordings?', a: 'No. ScreenCraft imposes no time limits. The practical limit is your device\'s available RAM — very long recordings require more memory. For recordings longer than 30 minutes, we recommend checking your device\'s free memory beforehand and closing other browser tabs to maximize available resources.' },
    ],
  },
  {
    category: 'Technical & Compatibility',
    items: [
      { q: 'Which browsers work with ScreenCraft?', a: 'Chrome and Edge (Chromium-based) offer the best support, including system audio capture. Firefox supports screen and microphone recording but has some limitations on system audio. Safari on macOS supports basic camera/mic recording but screen capture support is limited. We recommend Chrome for the most complete feature set, especially if you need system audio.' },
      ...
      { q: 'Is ScreenCraft completely free?', a: 'Yes, completely free. No free tier, no premium plan, no paywall. Every feature — HD recording, all recording modes, video trimming, audio trimming — is available to every user without payment or registration. ScreenCraft is supported by non-intrusive display advertising, which is how we keep the service free.' },
    ],
  },
];

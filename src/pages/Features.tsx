...
const features = [
  ...
  { icon: Video, title: 'Webcam Recording', desc: 'Record from your webcam as standalone content or as an overlay on your screen recording. ScreenCraft supports all USB and built-in webcams compatible with your browser.' },
  ...
  { icon: Infinity, title: 'No Recording Limits', desc: 'ScreenCraft imposes zero time limits on recordings. Record for as long as you need — the only limit is your device memory.' },
  { icon: Globe, title: '100% Browser Based', desc: 'No installation. No extension. No plugin. ScreenCraft runs entirely in modern browsers using standard Web APIs. Open the URL and start recording.' },
  ...
  { icon: Zap, title: 'Instant Start, No Login', desc: 'There\'s no account, no registration, no onboarding flow. Open ScreenCraft and click Record. That\'s it.' },
];

const comparison = [
  { feature: 'Free to use', screencraft: true, screenrecordercom: true, obs: true, loom: false },
  { feature: 'No login required', screencraft: true, screenrecordercom: true, obs: true, loom: false },
  { feature: 'No upload to servers', screencraft: true, screenrecordercom: false, obs: true, loom: false },
  { feature: 'Browser-based', screencraft: true, screenrecordercom: true, obs: false, loom: true },
  { feature: 'No watermarks', screencraft: true, screenrecordercom: false, obs: true, loom: false },
  { feature: 'Video trimmer', screencraft: true, screenrecordercom: false, obs: false, loom: true },
  { feature: 'System audio', screencraft: true, screenrecordercom: true, obs: true, loom: true },
  { feature: 'HD recording', screencraft: true, screenrecordercom: true, obs: true, loom: true },
  { feature: 'No install required', screencraft: true, screenrecordercom: true, obs: false, loom: true },
  { feature: 'No time limits', screencraft: true, screenrecordercom: false, obs: true, loom: false },
];

const cols = ['ScreenCraft', 'Screen-Recorder.com', 'OBS Studio', 'Loom'];
  ...
  return (
    <Layout>
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-16">
          <h2 className="text-2xl font-bold text-white mb-6">How ScreenCraft Compares</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    {cols.map(col => (
                      <th key={col} className={`text-center font-semibold px-4 py-3 ${col === 'ScreenCraft' ? 'text-primary' : 'text-white'}`}>
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {comparison.map(({ feature, screencraft, screenrecordercom, obs, loom }) => (
                    <tr key={feature} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                      ...
                      {[screencraft, screenrecordercom, obs, loom].map((val, i) => (
